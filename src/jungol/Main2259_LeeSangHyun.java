package jungol;/*
시골에 있는 태양이의 삼촌 댁에는 커다란 참외밭이 있다. 문득 태양이는 이 밭에서 자라는 참외가 도대체 몇 개나 되는지 궁금해졌다. 어떻게 알아낼 수 있는지 골똘히 생각하다가 드디어 좋은 아이디어가 떠올랐다. 유레카! 1㎡의 넓이에 자라는 참외 개수를 헤아린 다음, 참외밭의 넓이를 구하면 비례식을 이용하여 참외의 총개수를 구할 수 있다.
...
public class Main2259_LeeSangHyun {
	public static void main(String[] args) throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		int fullHeight = 0; // 전체 높이
		int shortHeight = 0; // 잘라내는 구간 높이
		int fullWidth = 0; // 전체 너비
		int shortWidth = 0; // 잘라내는 구간 너비
		...
		for (int i = 0; i < 6; i++) {
			...
			if (a[i][0] == 3 || a[i][0] == 4) { // 높이 먼저 세팅
				if (a[i][1] != fullHeight) { // 현재 높이가 전체 높이라면 패스
					// ㄴ 90도 대칭 모양: 잘라내는 구간
					if (a[i][0] == 3 && a[temp][0] == 2) {
						shortHeight = a[i][1];
						shortWidth = a[temp][1];
						break;
						// ㄱ 90도 대칭 모양: 잘라내는 구간
					} else if (a[i][0] == 4 && a[temp][0] == 1) {
						shortHeight = a[i][1];
						shortWidth = a[temp][1];
						break;
					}
				...
				if (a[i][1] != fullWidth) { // 현재 너비가 전체 너비라면 패스
					// ㄱ 모양: 잘라내는 구간
					if (a[i][0] == 1 && a[temp][0] == 3) {
						shortWidth = a[i][1];
						shortHeight = a[temp][1];
						break;
						// ㄴ 모양: 잘라내는 구간
					} else if (a[i][0] == 2 && a[temp][0] == 4) {
						shortWidth = a[i][1];
						shortHeight = a[temp][1];
						break;
					}
		...
		// 전체 넓이에서 잘라내는 구간 넓이를 빼고 참외 개수를 곱해 출력
		int fullSquare = fullWidth * fullHeight;
		int shortSquare = shortWidth * shortHeight;
		System.out.println((fullSquare - shortSquare) * k);
	}
}